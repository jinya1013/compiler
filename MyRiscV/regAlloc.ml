...
and g' dest cont regenv p = function (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
  ...
  | FMov(x) -> (Ans(FMovD(find x Type.Float regenv), p), regenv)
  | FNeg(x) -> (Ans(FNegD(find x Type.Float regenv), p), regenv)
  | FAdd(x, y) -> (Ans(FAddD(find x Type.Float regenv, find y Type.Float regenv), p), regenv)
  | FSub(x, y) -> (Ans(FSubD(find x Type.Float regenv, find y Type.Float regenv), p), regenv)
  | FMul(x, y) -> (Ans(FMulD(find x Type.Float regenv, find y Type.Float regenv), p), regenv)
  | FDiv(x, y) -> (Ans(FDivD(find x Type.Float regenv, find y Type.Float regenv), p), regenv)
