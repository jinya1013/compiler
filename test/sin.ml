...
let rec reduction_f t p = 
  if t >= p then reduction_f t (2.0 *. p) else reduction_g t p
in
let rec fsin t = 
  let (t, flg) = if t > 0.0 then (t, 1.0) else if t < 0.0 then (-1.0 *. t, -1.0) else (t, 0.0) in
  let t = reduction_f t two_pi in
  let (t, flg) = if t > pi then (t -. pi, -1.0 *. flg) else (t, flg) in
  let t = if t > pi_2 then pi -. t else t in
  if t > pi_4 then flg *. (kernel_sin t) else flg *.(kernel_cos (pi_2 -. t)) *)
let rec kernel_sin t = 
  t -. s3 *. t *. t*. t +. s5 *. t *. t *. t *. t *. t -. s7 *. t *. t *. t *. t *. t *. t *. t
in
