...
let rec sbst_var_name x new_x = function
  ...
  | LetTuple (xts, a, t, p) -> LetTuple (xts, sbst_id x new_x a, sbst_var_name x new_x t, p) (* 宣言する変数は書き換えない *)
  ...
  | ExtArray (a, p) -> ExtArray ((sbst_id x new_x a), p)
  | ExtFunApp(f, args, p) -> ExtFunApp((sbst_id x new_x f), List.map (fun e -> sbst_id x new_x e) args, p)
  | LetRec({ name = (f, t); args = yts; body = e1 }, e2, p) -> LetRec({ name = (f, t); args = yts; body = sbst_var_name x new_x e1 }, sbst_var_name x new_x e2, p) (* 宣言する変数は書き換えない *)
  | Let((a1, a2), e1, e2, p) -> Let((a1, a2), sbst_var_name x new_x e1, sbst_var_name x new_x e2, p) (* 宣言する変数は書き換えない *)
  | e -> e
